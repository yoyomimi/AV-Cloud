...
from libs.models.vigas.hyperconv import HyperConvBlock
from libs.models.vigas.warping import GeometricTimeWarper, MonotoneTimeWarper
...
class Warpnet(nn.Module):
    ...
    def forward(self, mono, view):
        ...
        warped = self.neural_warper(mono, neural_warpfield)
        return warped


class HyperConvWavenet(nn.Module):
    def __init__(self, z_dim, channels=64, blocks=3, layers_per_block=10, conv_len=2, remove_hyperconv=False,
                 acausal=False):
        super().__init__()
        self.layers = []
        self.rectv_field = 1
        for b in range(blocks):
            for l in range(layers_per_block):
                dilation = 3 ** l if acausal else 2 ** l
                self.layers += [HyperConvBlock(channels, channels, z_dim, kernel_size=conv_len, dilation=dilation,
                                               remove_hyperconv=remove_hyperconv, acausal=acausal)]
                self.rectv_field += self.layers[-1].receptive_field() - 1
        self.layers = nn.ModuleList(self.layers)
        print('Wavenet receptive field: ', self.rectv_field)
...
class WaveoutBlock(nn.Module):
    def __init__(self, channels, output_channel):
        super().__init__()
        ...
        self.second = nn.Conv1d(channels, output_channel, kernel_size=1)
