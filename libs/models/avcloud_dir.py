...
class AVCloudDir(nn.Module):
    """Baseline AVCloudDir
    """

    def __init__(self, cfg, gaussian_model, scene, intermediate_ch = 128, time_num = 126,
        freq_num = 257, dilation=1):

        super(AVCloudDir, self).__init__()
        ...
        self.vec_proj1 = nn.Sequential(
            nn.Linear(63, self.d_model//2),
            nn.PReLU()
        )
    ...
    def forward(self, cam_pose, source_gt_audio, input_bboxes, intri, is_val=False):
        """
        Args:
            ...
            input_bboxes: (B, 4). The box of active speaker.
            intri: (B, 3, 3). Intrisic camera parameters.
        Return:
        ...
        # View direction encoding based on target active speaker
        input_center = (input_bboxes[..., :2] + input_bboxes[..., 2:]) / 2.
        point_view = torch.cat([input_center, torch.ones(B, 1).to(device)], -1)
        point_view = point_view.unsqueeze(1) @ intri.inverse().permute(0, 2, 1)
        image2world = torch.eye(4).to(device).repeat(B, 1, 1)
        image2world[:, :3, :3] = camera_R
        image2world[:, :3, 3] = camera_center
        world_point_homogeneous = torch.bmm(torch.cat([point_view, torch.ones(B, 1, 1).to(device)], -1).float(), image2world.permute(0, 2, 1).float())
        ori = world_point_homogeneous[..., 0, :3] - camera_center
...
def build_model(cfg, gaussian_model, scene):
    model = AVCloudDir(cfg, gaussian_model, scene)
    return model